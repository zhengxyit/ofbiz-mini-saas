        ...
        if (UtilValidate.isNotEmpty(fieldsToSelect)) {
            ...
            for (String fieldToSelect : fieldsToSelect) {
                if (tempKeys.contains(fieldToSelect)) {
                    ...
                    if (curField != null) {
                        if (!curField.getIsSupportModules() || (curField.getIsSupportModules() && delegator.checkEnterpriseHasModules(curField.getSupportModules()))) {
            ...
            List<ModelField> sfs = new ArrayList<ModelField>();
            // 去掉不支持的字段
            for (ModelField curField : selectFields) {
                if (!curField.getIsSupportModules() || (curField.getIsSupportModules() && delegator.checkEnterpriseHasModules(curField.getSupportModules()))) {
                    sfs.add(curField);
                }
            }
