...
public class GenericDelegator implements Delegator {
    ...
    private String enterpriseModules = null;// ALL代表所有
    ...
    protected GenericDelegator(String delegatorFullName) throws GenericEntityException {
        ...
        String kekText = this.delegatorInfo.getKeyEncryptingKey();
    ...
    @Override
    public String getEnterpriseModules() {
        return this.enterpriseModules;
    }

    @Override
    public void setEnterpriseModules(String supportModules) {
        this.enterpriseModules = supportModules;
        if ("".equals(this.enterpriseModules)) {
            this.enterpriseModules = null;
        }

        // 如果前后没有,号就加上
        if (this.enterpriseModules != null && !"ALL".equals(this.enterpriseModules)) {
            if (',' != this.enterpriseModules.charAt(0)) {
                this.enterpriseModules = "," + this.enterpriseModules;
            }
            if (',' != this.enterpriseModules.charAt(this.enterpriseModules.length() - 1)) {
    ...
    private void checkTenant(GenericValue value) throws GenericEntityException {
        // 2017-3-22 加多租户判断 在启用事务外 启用了没有要抛异常
        if (EntityUtil.isMultiTenantEnabled() && value.getModelEntity().getIsTenant()) {
            if (this.tenantId == null) throw new GenericEntityException("[tenant] not found");
        }
    }

    private void checkTenant(GenericValue value, GenericValue checkValue) throws GenericEntityException {
        // 2017-3-22 加多租户判断
        if (EntityUtil.isMultiTenantEnabled() && value.getModelEntity().getIsTenant()) {
            if (!this.tenantId.equals(checkValue.get(ModelEntity.TENANT_FIELD)))
                throw new GenericEntityException("[tenant] not found");
        }
    }

    private EntityCondition makeTenantInCondition(EntityCondition condition, ModelEntity modelEntity) {
        // 2017-3-22 Tenant需要加一个条件判断
        if (EntityUtil.isMultiTenantEnabled() && modelEntity.getIsTenant()) {
            EntityCondition expr = EntityCondition.makeCondition(ModelEntity.TENANT_FIELD, EntityOperator.EQUALS, this.tenantId);
            if (condition == null) {
                return expr;
            } else {
                List<EntityCondition> exprList = new ArrayList();
                exprList.add(expr);
                exprList.add(condition);
    ...
    private Set<String> makeModulesFieldsToSelect(Set<String> fieldsToSelect, ModelEntity modelEntity) {
        ...
        while (it.hasNext()) {
            ...
            if (modelField.getSupportModules() == null) {
                fieldsToSelect.add(modelField.getName());
            } else {
                if (this.checkEnterpriseHasModules(modelField.getSupportModules())) {
                    fieldsToSelect.add(modelField.getName());
                }
            }
        }
    ...
    public GenericValue createOrStore(GenericValue value) throws GenericEntityException {
        ...
        try {
            ...
            this.checkTenant(value, checkValue);
    ...
    public int removeByCondition(String entityName, EntityCondition condition) throws GenericEntityException {
        ...
        try {
            ...
            // 将Tenant的条件放进去
            condition = this.makeTenantInCondition(condition, modelEntity);
    ...
    public List<GenericValue> findList(String entityName, EntityCondition entityCondition, Set<String> fieldsToSelect, List<String> orderBy, EntityFindOptions findOptions, boolean useCache) throws GenericEntityException {
        ...
        // 检查是否有该模块功能
        boolean hasModules = checkEnterpriseHasModules(modelEntity.getSupportModules());
        if (!hasModules) {
            String msg = "The Enterprise[" + this.tenantId + "] is not support [" + this.enterpriseModules + "];";
            Debug.log(msg);
            throw new GenericEntityException(msg);
        }

        // 将Tenant的条件放进去
        entityCondition = this.makeTenantInCondition(entityCondition, modelEntity);
    ...
    public List<GenericValue> getRelated(String relationName, Map<String, ? extends Object> byAndFields, List<String> orderBy, GenericValue value, boolean useCache) throws GenericEntityException {
        ...
        if (relation.getIsSupportModules() && !this.checkEnterpriseHasModules(relation.getSupportModules())) {
            return null;
        }
    ...
    public boolean checkEnterpriseHasModules(String modules) {
        if (this.enterpriseModules == null) {
            // 为NULL时不限制，所有企业默认是空字符串
            return true;
        }
        if ("".equals(this.enterpriseModules)) {
            return false;
        }
        if ("ALL".equals(this.enterpriseModules) || modules == null || "".equals(modules)) {
            return true;
        }

        String[] codes = modules.split(",");
        for (String code : codes) {
            if (!this.enterpriseModules.contains(code)) {
                return false;
            }
        }
